...
def extract_pairwise_features(s1_rec: dict, cand_rec: dict) -> List[float]:
    ...
    # RapidFuzz scores (scaled to 0.0 - 1.0)
    name_ratio = fuzz.ratio(s1_ns, c_ns) / 100.0 if (s1_ns and c_ns) else 0.0
    name_partial = fuzz.partial_ratio(s1_ns, c_ns) / 100.0 if (s1_ns and c_ns) else 0.0
    name_sort = fuzz.token_sort_ratio(s1_ns, c_ns) / 100.0 if (s1_ns and c_ns) else 0.0
    name_set = fuzz.token_set_ratio(s1_ns, c_ns) / 100.0 if (s1_ns and c_ns) else 0.0
    name_wratio = fuzz.WRatio(s1_ns, c_ns) / 100.0 if (s1_ns and c_ns) else 0.0
    name_jw = distance.JaroWinkler.similarity(s1_ns, c_ns) if (s1_ns and c_ns) else 0.0
    ...
    if not addr_missing:
        ...
        s1_digs = set(s1_rec.get('addr_digits', ''))
        c_digs = set(cand_rec.get('addr_digits', ''))
